...
(function() {
				...
				.bind('touchstart' + (suffix ? '.' + suffix : ''), function() {
					if (!isTapLocked) {
						...
						if (_settings.mobileUI_tapDelay > -1)
							tapId = window.setTimeout(function() {
	...
	jQuery.fn.disableSelection_5grid = function() {
		return jQuery(this).css('user-select', 'none').css('-khtml-user-select', 'none').css('-moz-user-select',
			'none').css('-o-user-select', 'none').css('-webkit-user-select', 'none');
	}
	jQuery.fn.enableSelection_5grid = function() {
		return jQuery(this).css('user-select', 'auto').css('-khtml-user-select', 'auto').css('-moz-user-select',
			'auto').css('-o-user-select', 'auto').css('-webkit-user-select', 'auto');
	}
	jQuery.fn.accelerate_5grid = function() {
		return jQuery(this).css('-webkit-transform', 'translateZ(0)').css('-webkit-backface-visibility',
			'hidden').css('-webkit-perspective', '500');
	}
	...
	(function() {
		...
		// Mobile (exclusive)
		if ((doMobile && (ww <= _settings.breakpoint_mobile || sw <= _settings.breakpoint_mobile)) ||
			doMobileOnly) {
			...
			if (_settings.mobileUI == 1) {
				...
				if (_settings.mobileUI_theme != 'none') {
					...
					if (_settings.mobileUI_themeTitleBarColor)
						headQueue.pushI_5grid('#mobileUI-site-titlebar { background-color: ' + _settings
							.mobileUI_themeTitleBarColor + '; }');

					if (_settings.mobileUI_themeNavColor)
						headQueue.pushI_5grid('#mobileUI-site-nav { background-color: ' + _settings
							.mobileUI_themeNavColor + '; }');
				}
			}
		...
		jQuery(function() {
			...
			jQuery('.mobileUI-site-nav a').each(function() {
				...
				site_nav_options.push(
					'<div class="mobileUI-site-nav-link mobileUI-site-nav-link-depth-' + indent +
					'" xhref="' + t.attr('href') + '"><span class="indent-' + indent + '"></span>' +
					t.text() + '</div>'
				);
			});
			...
			if (site_nav_options.length > 0) {
				_nav_inner = jQuery('<div id="mobileUI-site-nav-inner"><nav>' + site_nav_options.join('') +
					'</nav></div>');
				_nav = jQuery('<div id="mobileUI-site-nav"></div>');
				_nav_inner.appendTo(_nav);
				_opener = jQuery('<div id="mobileUI-site-nav-opener">' + _settings.mobileUI_openerText +
					'</div>');
			} else {
				_nav_inner = jQuery();
				_nav = jQuery();
			...
			_titlebar = jQuery('<div id="mobileUI-site-titlebar"><div id="mobileUI-site-title">' + site_name +
				'</div></div>');
			...
			if (_settings.mobileUI_titleBarOverlaid)
				...
				.bind('resize', function() {
					_viewer.height(_content.height() + _settings.mobileUI_titleBarHeight);
				});
